...
class ClipCocoDataset(Dataset):
    ...
    def pad_tokens(self, data, item: int):
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, ...]:
        with open(f"{self.data_folder}{item}.pkl", 'rb') as f:
            data = pickle.load(f)
        tokens, mask = self.pad_tokens(data, item)
        # print("tokens: ",tokens)
        # print("mask: ",mask)
        # print("decoded tokens: ",self.tokenizer.decode(tokens))
        prefix = data["clip_embedding"]#self.prefixes[self.caption2embedding[item]]
    ...
    def __init__(self, data_path: str,  prefix_length: int, gpt2_type: str = "gpt2",
                 normalize_prefix=False, is_eng=True, shots_count=None):
        # if is_eng:
        ...
        with open(f"{self.data_folder}dataset_info.pkl", 'rb') as f:
            d = pickle.load(f)
            self.max_seq_len = d["processed_max_seq_len"]
            self.dataset_len = d["dataset_length"]
